...
Establishing which changes were real required a validation harness, which also measured
properties of the evaluation itself: a divisor that dominates score variance and is recoverable
in closed form from a published baseline row, and $70.4\%$ advertiser overlap across
channel-disjoint splits. Those findings are reported in \S\ref{sec:evaluation} as feedback to
the organisers rather than as the paper's subject.
...
(\S\ref{sec:hand}). The score reported throughout as the system's is $0.6537$, the returned
score of the last entry containing no hand-set label. No rank is claimed.
...
(macro-F1)''}, which leaves open how a class absent from both gold and predictions is treated,
a known ambiguity in reported macro-F1 \cite{opitz2019macro,opitz2024closer}. Scoring the
organisers' published constant-class baseline under a $2\times2$ grid of label-set and
zero-division conventions, only the \emph{present-label-set} convention, which averages over
classes appearing in the gold or the predictions of the scored rows, reproduces the published
row (ST1 $0.150970$ against $0.1510$; a fixed five-class average gives $0.120776$). It is also
\texttt{sklearn}'s default when \texttt{labels} is unset \cite{pedregosa2011scikit}. Everything
below uses this pinned convention, under which the divisor of each column depends on the sample
...
count selected once per run, never per fold. Inputs are assembled under per-field token budgets so that a long transcript cannot evict the
product-page text, and non-transcript fields are dropped at $p{=}0.15$ during training so that
no single field becomes load-bearing.
...
$0$; a data-guard instruction directs the model to treat the instance as data rather than as
instructions. Per-label probabilities
...
rebuilt from the four members able to separate that class. Both adjustments apply identically during fitting and emission.
...
(ST1 $0.0869$, ST2 $0.0254$, ST3 $0.0446$). None of the four adjacent gaps in the published top
five ($0.0087$, $0.0181$, $0.0057$, $0.0323$) reaches one standard error of the estimate that
produced them \cite{card2020power,dror2018hitchhiker}, so this paper does not interpret its own
...
neural member plus a CPU stage. The recorded figure is the zero-shot pass, the largest model in
the system: $14.4$ minutes on a single H200, including model load, covering two prompt variants
across all three sub-tasks. The encoder and LoRA members are forward passes of smaller models
over the same instances, run inside their training jobs, and add minutes rather than hours at
those jobs' recorded throughputs; the CPU stage emits final labels from stored member
probabilities in $0.26$ seconds on a laptop. A conservative estimate for the full ensemble is
under one GPU-hour; retaining only the zero-shot member and the decision layer, which
...
entirely on ST1 ($+0.089$) and ST2 ($+0.096$), the two sub-tasks concerned with what is being
sold, while ST3 is flat at every step, $+0.024$ from L1 to L4, with every adjacent interval
spanning zero. On this
...
contribution inside one ensemble with two of six intervals spanning zero, but here the prompted
member contributed more than the fine-tuned encoders at a quarter of the compute.
...
macro expected F1 are different objectives under present-label semantics. Legal provisions in
the prompt were not measured here; Gui et al. \shortcite{gui2025explanations} report at this
venue that they improve explanation quality without consistently improving detection.
...
itself. They are summarised here as feedback for future editions and detailed in
Appendix~\ref{app:evaluation}; none is a claim about any other entry.
...
and conditioning on that event splits the bootstrap distribution into two modes: mean metric
$0.6976$ (s.e. $0.0090$) with the class present and $0.7506$ (s.e. $0.0102$) without, on
bit-identical predictions (Figure~\ref{fig:divisor}, Appendix~\ref{app:repro}). The $+0.0530$
gap is about six times the $0.0087$ separating the first and second entries on the final board,
and whether to emit such a class is undecidable from the training data, since which error is
worse depends on the hidden gold. That decision was made incorrectly here, at the cost
\S\ref{sec:invert} states.
...
or does not. The organisers
published a constant \lab{physical\_goods} baseline for both phases. On the development split
($S=0.1510$, $N=504$) only $K=4$ admits a solution, $g=218$, which is exactly the development
gold. On the evaluation split ($S=0.1274$, $N=503$) two candidates survive, $K=4$ with $g=172$
and $K=5$ with $g=235$; the pooled labelled rate of $0.4701$ selects the second, and the
overview paper's Table 3 confirms $235$ \lab{physical\_goods} and one \lab{other}. The class
this system suppresses was therefore present, and the suppression cost a fifth of the ST1
column ($0.0676$ of the mean, \S\ref{sec:hand}) across three of five submission slots.
Appendix~\ref{app:evaluation} relates this to prior label-recovery results and lists remedies;
fixing the label set for the average removes both this and the variance above in one step.
...
labelled data and $70.4\%$ of evaluation rows carry one. On the development split, a lookup
table that copies an advertiser's majority training label, with no model at all, scores
$0.4649$ mean macro-F1 against $0.0928$ for the majority baseline and $0.7352$ for the full
system, and assigns ST1 exactly right on $93.0\%$ of the $358$ rows whose advertiser is
labelled. This is not evidence that systems perform lookup, and this one does not (development
score $0.7126$ on unseen advertisers against $0.6805$ on seen). The shortcut
is far weaker for compliance ($+0.283$ on ST3 against $+0.534$ on ST1 and $+0.657$ on ST2 over
the majority baseline), the same asymmetry the data-level ablation shows: what is sold is
substantially memorisable, whether it complies is not.
...
earlier entry, withdrawn before the final one.
...
one labelled segment (PrizePicks) and one with none (Picklebet). The fourth, in the earlier
entry, converted a genuine \lab{physical\_goods} instance to \lab{other} against a seven-to-one
...
that this holds for two, is absent for the Picklebet label, whose advertiser that account also
misnamed, and is contradicted by the fourth. Both corrections were sent to the organisers.
...
board, for the reason \S\ref{sec:divisor} describes: ST1 \lab{other} has exactly one gold
instance in the evaluation split, so one label constituted the entire gold column for its class
and moved one fifth of the ST1 column from zero to full credit.

\paragraph{What the record can establish.}
The evaluation phase scores an uploaded predictions file and collects a fact sheet; the
producing code is not collected, so no retained artefact distinguishes a system's prediction
from one entered by hand, and the sentence volunteered on this entry's fact sheet was the only
trace of these edits. No evidence is held about any other entry, in either direction. No check
that would have identified them (collect the code, rerun it, diff against the submitted file)
was run, so what is known about provenance is what teams chose to report.
...
applied uniformly to all $503$ evaluation instances it selects exactly that one, and the
resulting file would return $0.7213$. That is evidence the label was reachable within the
rules, not an achieved score: the file was never submitted, and the rule was written knowing
...
evaluation score is $0.6537$. Three observations may transfer: a prompted member contributed
more than the fine-tuned encoders at a quarter of the compute, and one six-GPU-hour encoder was
removable at no measurable cost; measured on the zero-shot member, the channel name added
nothing and the product-page crawl supported product identification rather than compliance
judgement; and the compliance sub-task appears label-limited. The final submitted
entry contained three hand-set labels, documented in \S\ref{sec:hand}; the system-only score is
...
$0.0200$ is roughly twice the within-regime figure, and none of the excess reflects a property
of the system. Two rows of a class no system emits outweigh the gap between the two leading
...
lookup's gains over the majority baseline are $+0.534$ on ST1, $+0.657$ on ST2 and $+0.283$ on
ST3; the advertiser proxy will merge distinct advertisers that share a platform domain and split
advertisers across country domains. A future edition might report an advertiser-disjoint split
